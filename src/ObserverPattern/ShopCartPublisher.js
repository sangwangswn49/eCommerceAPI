...
class ShopCartPublisher {
    ...
    // Modules cause trigger for notification
    addShopCart = (data) => {
            ...
            try {
                if (!data.userId || !data.productdetailsizeId || !data.quantity) {
                    ...
                    if (cart) {
                        ...
                        this.notifyObservers(data.userId);
                    }
                    else {
                        let res = await db.ProductDetailSize.findOne({ where: { id: data.productdetailsizeId } })
                        if (res) {
...
export default ShopCartPublisher;